...
def test_rfsq():
    ...
    residual_fsq = ResidualFSQ(dim=256, levels=[8, 5, 5, 3], num_quantizers=8)
...
@pytest.mark.parametrize("codebook_scale", (1.0, 0.5))
def test_lfq(spherical, codebook_scale):
...
def test_lfq_video():
    ...
    quantizer = LFQ(
        codebook_size=65536, dim=16, entropy_loss_weight=0.1, diversity_gamma=1.0
    )
...
def test_rflq():
    ...
    residual_lfq = ResidualLFQ(dim=256, codebook_size=256, num_quantizers=8)
